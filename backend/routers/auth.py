...
async def login_microsoft():
    microsoft_auth_url = (
        ...
        f"redirect_uri=http://localhost:8000/api/auth/microsoft&"
        f"scope=openid email profile&"
        f"response_type=code"
    )
...
async def auth_microsoft(code: str = None):
    if code:
        return RedirectResponse(url="http://localhost:3000/login/success?provider=microsoft")
